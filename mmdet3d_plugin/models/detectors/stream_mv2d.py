...
class StreamMV2D(Base3DDetector):
    ...
    def forward_train(self,
        ...
        batch_size, ql, num_views, c, h, w = img.shape
        if self.use_grid_mask:
            img = self.grid_mask(img.view(batch_size * ql * num_views, c, h, w))
            img = img.view(batch_size, ql, num_views, c, h, w)

        assert batch_size == 1, 'only support batch_size 1 now'

        cur_img = img[:, -self.num_frame_losses:]
        ...
        ori_cur_img_metas, ori_gt_bboxes_3d, ori_gt_labels_3d, ori_gt_bboxes_ignore = (
            img_metas[0][-self.num_frame_losses:], gt_bboxes_3d, gt_labels_3d, gt_bboxes_ignore)
        cur_gt_bboxes, prev_gt_bboxes, cur_gt_labels, prev_gt_labels, gt_bboxes_3d, gt_labels_3d, gt_bboxes_ignore, cur_img_metas, = [], [], [], [], [], [], [], []
        for i in range(batch_size):
            cur_img_metas_views = ori_cur_img_metas[i]

            for j in range(num_views):
                img_meta = dict(num_views=num_views)
                for k, v in cur_img_metas_views.items():
            ...
            cur_gt_bboxes.extend(gt_bboxes_2d[i][-self.num_frame_losses:][0])
            cur_gt_labels.extend(gt_labels_2d[i][-self.num_frame_losses:][0])
            gt_bboxes_ignore.extend(ori_gt_bboxes_ignore[i])

        # calculate losses for 2D detector
        detector_feat = self.extract_feat(cur_img)
        ...
        with torch.no_grad():
            results = self.base_detector.simple_test_w_feat(detector_feat, cur_img_metas)

        # process 2D detection
        detections = self.process_2d_detections(results, img.device)
        if self.train_cfg.get('complement_2d_gt', -1) > 0:
            detections_gt = self.process_2d_gt(cur_gt_bboxes, cur_gt_labels, img.device)
        ...
        roi_losses = self.roi_head.forward_train(feat, cur_img_metas, detections,
                                                 cur_gt_bboxes, cur_gt_labels,
                                                 gt_bboxes_3d, gt_labels_3d,
                                                 ori_gt_bboxes_3d, ori_gt_labels_3d,
                                                 attr_labels, None, **data)
        losses.update(roi_losses)
        return losses

    def forward_test(self, img, img_metas, **data):
        num_augs = len(img)
        ...
        if num_augs == 1:
            return self.simple_test(img[0], img_metas[0], **data)
        else:
            return self.aug_test(img, img_metas, **data)

    def simple_test(self, img, img_metas, proposal_bboxes=None, proposal_labels=None, rescale=False, **data):
        ...
        # generate 3D detection
        bbox_outputs_all = self.roi_head.simple_test(feat, detections, img_metas, rescale=rescale, **data)
        # end_time = time.time()
