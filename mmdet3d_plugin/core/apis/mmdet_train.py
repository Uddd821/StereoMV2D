...
from mmcv.runner.hooks import HOOKS, Hook, IterTimerHook, TextLoggerHook
...
def custom_train_detector(model,
    ...
    runner.log_buffer.output.setdefault('time', 0.0)
    runner.log_buffer.output.setdefault('data_time', 0.0)

    # 手动注册 Timer（最高优先级）
    runner.register_hook(IterTimerHook(), priority='HIGHEST')

    # 手动注册“安全版”Logger（最低优先级）
    runner.register_hook(
        SafeTextLoggerHook(by_epoch=False, ignore_last=False, reset_flag=True),
        priority='VERY_LOW'
    )

    # 让框架别再自动塞 Logger/Timer（防重复/防顺序不可控）
    runner.register_training_hooks(
        cfg.lr_config,
        cfg.optimizer_config,
        cfg.checkpoint_config,
        log_config=None,  # 关闭自动 logger
