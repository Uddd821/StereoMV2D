...
@PIPELINES.register_module()
class DefaultFormatBundleMono3Dv2(DefaultFormatBundle3D):
    def __init__(self, class_names, collect_keys, with_gt=True, with_label=True):
        super(DefaultFormatBundleMono3Dv2, self).__init__(class_names, collect_keys)
        self.class_names = class_names
        self.with_gt = with_gt
        self.with_label = with_label
        self.collect_keys = collect_keys
    def __call__(self, results):
        results = super(DefaultFormatBundleMono3Dv2, self).__call__(results)
...
class CollectMono3D(Collect3D):
    def __init__(
        ...
        meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img',
                   ...
                   'affine_aug', 'intrinsics', 'extrinsics', 'timestamp', 'img_timestamp'),
        debug=False,
